...
export const updateAirline = async (req, res)=>{
  ...
  console.log(airline)
  if(mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send('Airline is not found')

  try {
    updatedAirline = await Airline.findByIdAndUpdate(_id, airline, {new: true})
