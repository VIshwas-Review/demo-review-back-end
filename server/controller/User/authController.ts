...
export const signIn = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!isValidPassword) return res.status(400).json({ message: 'Invalid credential' })
...
export const signUp = async (req: Request, res: Response) => {
  const { email, password, confirmPassword, imageUrl, firstName, lastName, role } = req.body
  ...
  try {
    ...
    if (password !== confirmPassword)
      return res.status(400).json({ message: 'Confirm Password is not matching with password' })
